"""Training pipeline for ALBERT-based spam classification with fine-tuning.

This experiment fine-tunes an ALBERT model from scratch for spam classification.
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description='Fine-tune ALBERT model for spam classification')
    parser.add_argument(
...
def main(args):
    """Load dataset, fine-tune ALBERT, and evaluate on all splits."""
    # ===== DATASET / PIPELINE =====
    dataset = CommentsDataset()

    bert = BERTClassifier(model_name='albert-base-v2')
    ...
    # ========== FINAL SUMMARY ==========
    experiment = 'bert-albert-finetuned'
